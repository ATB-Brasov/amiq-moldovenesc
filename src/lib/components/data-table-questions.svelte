	...
	export const columns: ColumnDef<Schema>[] = [
		{
			...
			header: ({ table }) =>
				renderComponent(DataTableCheckbox, {
					...
					onCheckedChange: (value) => console.log("table.toggleAllPageRowsSelected(!!value)"),
					"aria-label": "Select all",
				}),
            cell: ({row}) => 
                renderComponent(DataTableCheckbox, {
                checked: row.original.selectat,
                onCheckedChange: (value) => console.log("row.toggleSelected(!!value)"),
		...
		},
		{
			accessorKey: "titlu",
			header: "Titlu",
			cell: ({ row }) => renderSnippet(DataTableText, { text: row.original.titlu }),
			enableHiding: false,
		},
		{
			accessorKey: "echipa",
...
<script lang="ts">
	...
	import type { QSchema as Schema } from "./schemas.js";
	...
	import DataTableCellViewer from "./data-table-questions-cell-viewer.svelte";
...
{#snippet DataTableText({ text }: { text: string })}
    <div>{text}</div>
{/snippet}
